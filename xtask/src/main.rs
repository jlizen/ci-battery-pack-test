...
/// Example codegen task. Replace with your own generated files.
fn codegen(check: bool) -> anyhow::Result<()> {
    let root = project_root();
    let _sh = Shell::new()?;

    // TODO: Add your codegen logic here.
    // Example: read a schema, generate Rust code, write to a file.
    // In --check mode, compare generated output with committed files
    // and panic if they differ.
...
fn tidy() -> anyhow::Result<()> {
    ...
    let output = cmd!(sh, "grep -rn --include=*.rs [[:space:]]$$ src/").ignore_status().read()?;
    if !output.is_empty() {
